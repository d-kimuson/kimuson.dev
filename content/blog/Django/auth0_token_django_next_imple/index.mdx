...
今回の実装だと XSS に対して被害がないということはもちろんなく、スクリプトを挿入できる時点で `auth.ts` と同じ処理をかけばトークンが取得できてしまうことになります(ローカルストレージから取るよりは遥かに面倒だとは思いますが)

とはいえ、ローカルストレージに置く場合には `トークンのリセット期間` = `ユーザーが再ログインする期間` になってしまうので、セキュリティ的な強さと UX がトレードオフになります。今回の手法ならセッションでログイン状態は維持できるのでトークンのリセットを高頻度で行うことができて良さそうだと思いました
