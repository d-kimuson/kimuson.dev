...
触ってみましたが、基本的には drf_yasg と同じ仕様感でした
...
API を使うには、書き出された `api.ts` を使います

- ApiAPi
- ApiAPiFp
- ApiApiFactory

辺りを使えますが、一番直感的だったので今回は ApiApiFactory を使います
...
を `extend_schema` デコレータで drf_spectacular に教えて上げる必要があります
...
APIを通してもインタフェースを共通化するために、フロントバック共に TypeScript を使うみたいな話をよく聞きますが、Django でも似たような体験で開発ができそうです
