...
これで、親に `m-context` を渡してやって、優先したい要素に `m-z1` 、それ以外の要素にはスコープを閉じるために `m-context` を付与してやれば良いことになります
...
これで、`l-header > l-main = l-footer` かつ main 下や footer 下の要素が header より上に来ることができなくなりました

ただし、ここに固定ヘッダより上に表示されるモーダルウィンドウを追加したいとなったとき、このやり方だとモーダルの優先順位を固定ヘッダより上に持ってくることができません
