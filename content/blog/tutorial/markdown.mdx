...
`lang:title` のように書くことで、コードブロックに任意のタイトルをつけることができます
...
[react-live](https://github.com/FormidableLabs/react-live) を使って、編集と実行を試すことができるコードブロックを使うことが出来ます

``` js react-live
const onClick = () => alert(`This is called from live code!`)
render(<button onClick={onClick}>Click Me!</button>)
```

## React Component を埋め込む

`mdx` ファイルでは、React Component の埋め込みをサポートします

import { TagList } from "@mdx"

<TagList tags={[`Markdown`, `ブログ`]} isLink />
