...
TypeScript には、型レベルで条件分岐ができる Conditional Types という型があり、Conditional Types の infer キーワードを使うことで特定のパターンにマッチする型を抽出することができます
...
Res1 では、T に string\[\] が渡るので、string 型変数 I に渡り、string 型が帰ります
...
(() => string) & (() => number) なんて型は意図して作らないとなかなか自然にはできないと思います

ただ、Union 型からスタートするなら、この辺の型はだいたい作ることができます

### Union から 共用体ベースの集約パターンを作る

Conditional Types では、評価される型が型変数で、かつ共用体型のとき、型が分配される Distributive Conditonal Types という性質があります
...
まとめると A | B みたいな型を持っている時、これらの挙動を利用して B 型を取り出すことと A & B 型を取り出せることがわかりました

記事的にはこれで終わりでも良いんですが、せっかくなのでこれらから作れる便利な型をいくつか紹介します
