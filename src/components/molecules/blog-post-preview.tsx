...
const Inner: React.FC<BlogPostPreviewProps> = ({ blogPost }: BlogPostPreviewProps) => {
  return <>
    <div className={styles.imageWrapper}>
      ...
      ) : <SiteLogo siteName={blogPost.siteName} className={styles.image} imgStyle={imgStyle} />}
    </div>
    ...
    <div className={styles.infoContainer}>
      ...
      {blogPost.__typename === `BlogPost` ?
        <TagList tags={blogPost.tags} /> : null
      }
    </div>
  </>
}
...
export const BlogPostPreview: React.FC<BlogPostPreviewProps> = ({
  ...
  return blogPost.__typename === `BlogPost`
    ...
    </Link> : <a href={blogPost.slug} className={`${styles.blogPost} m-card l-main-width m-remove-a-decoration`}>
      <Inner blogPost={blogPost} />
    </a>
}
